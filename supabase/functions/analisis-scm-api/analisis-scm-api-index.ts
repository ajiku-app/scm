...
// KPI logistics dihitung di server lewat RPC get_logistics_kpi (migrasi
// logistics_kpi_server_side): SLA %, rata-rata/median/p90 durasi, breakdown
// per armada, dll — tanpa perlu narik baris mentah lewat PostgREST.
// Default: seluruh data historis, SLA target 90 menit.
async function kpiLogistics() {
  const { data, error } = await supabase.rpc("get_logistics_kpi", {
    p_from: null,
    p_to: null,
    p_sla_menit: 90,
  });
  if (error) throw new Error(`get_logistics_kpi: ${error.message}`);
  return data ?? null;
}
